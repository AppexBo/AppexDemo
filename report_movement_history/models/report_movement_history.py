    ...
    def _process_moves(self, products, all_moves, date_start, location):
        for product in products:
            ...
            if moves:
                ...
                #), 4)
                balance = round(sum(
                    -line.product_uom_id._compute_quantity(line.quantity, line.product_id.uom_id) if line.location_id.id == location 
                    else line.product_uom_id._compute_quantity(line.quantity, line.product_id.uom_id)
                    for line in moves
                    if line.date <= date_start
                ), 4)
                ...
                for line in moves:
                    ...
                    # Convertir cantidad a la unidad de medida principal del producto
                    quantity_in_product_uom = line.product_uom_id._compute_quantity(line.quantity, line.product_id.uom_id)
                    ...
                    if location == line.location_id.id:
                        ...
                        temp_dict['out'] = quantity_in_product_uom 
                        balance -= quantity_in_product_uom  # Restar del balance
                        total_out += quantity_in_product_uom
                    else:
                        ...
                        temp_dict['in'] = quantity_in_product_uom 
                        balance += quantity_in_product_uom  # Sumar al balance
                        total_in += quantity_in_product_uom
